		...
		if (dao.isTableExists()) {
			...
			if (!results.isEmpty()) {
				...
				List<Metadata> queryMetadataList = dao.queryForEq(
						Metadata.COLUMN_SCOPE,
						metadata.getMetadataScope().getName());
				...
				fieldValues.put(Metadata.COLUMN_SCOPE,
						metadata.getMetadataScope().getName());
		...
		if (dao.isTableExists()) {
			...
			if (!results.isEmpty()) {
				...
				ub.updateColumnValue(Metadata.COLUMN_METADATA,
						updatedMetadataText);
				ub.where().eq(Metadata.COLUMN_MIME_TYPE,
						metadata.getMimeType());
				...
				for (Metadata preparedUpdateMetadata : results) {
					if (preparedUpdateMetadata.getMimeType()
							.equals(metadata.getMimeType())) {
		...
		if (dao.isTableExists()) {
			...
			if (!results.isEmpty()) {
				...
				for (MetadataReference metadataReference : metadataReferenceList) {
					TestCase.assertNotNull(metadataReferenceDao.queryByMetadata(
				...
				if (!results.isEmpty()) {
					...
					referenceCount = metadataReferenceDao
							.queryByMetadata(metadata.getId()).size();
					...
					if (cascade) {
						TestCase.assertTrue(queryMetadataReference.isEmpty());
						TestCase.assertTrue(
								queryMetadataParentReference.isEmpty());
					} else {
						TestCase.assertEquals(referenceCount,
								queryMetadataReference.size());
						TestCase.assertEquals(referenceParentCount,
					...
					for (MetadataReference metadataReference : metadataReferenceList) {
						TestCase.assertNotNull(metadataReferenceDao
