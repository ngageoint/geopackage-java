...
public class FeatureCacheUtils {
	 ...
	 *             upon error
	 */
	public static void testCache(GeoPackage geoPackage) throws SQLException {
		testCache(geoPackage, FeatureIndexType.GEOPACKAGE);
		testCache(geoPackage, FeatureIndexType.RTREE);
	}

	private static void testCache(GeoPackage geoPackage, FeatureIndexType type)
			throws SQLException {
		...
		for (String featureTable : featureTables) {
			...
			for (long featureRowId : featureIndexResults.ids()) {
				FeatureRow featureRow = featureCache.get(featureTable,
						featureRowId);
				...
				FeatureRow featureRow2 = featureCache.get(featureTable,
						featureRowId);
		...
		for (String featureTable : featureTables) {
			...
			for (long featureRowId : featureIndexResults.ids()) {
				...
				FeatureRow featureRow2 = featureCache2.get(featureTable,
						featureRowId);
				TestCase.assertNotNull(featureRow2);
				TestCase.assertEquals(featureRow, featureRow2);
			}
		...
		for (String featureTable : featureTables) {
			...
			for (long featureRowId : featureIndexResults.ids()) {
				FeatureRow featureRow = featureCache2.get(featureTable,
						featureRowId);
				TestCase.assertNotNull(featureRow);
				featureCache.put(featureRow);
			}
		...
		for (String featureTable : featureTables) {
			...
			long resultsCount = featureIndexResults.count();
			TestCase.assertEquals(resultsCount,
					featureCache.getSize(featureTable));
			TestCase.assertEquals(resultsCount,
					featureCache2.getSize(featureTable));
			for (long featureRowId : featureIndexResults.ids()) {
