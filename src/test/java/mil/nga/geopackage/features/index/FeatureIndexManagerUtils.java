...
public class FeatureIndexManagerUtils {
	 ...
	 *             upon error
	 */
	public static void testIndexChunk(GeoPackage geoPackage)
			throws SQLException {
		testIndexChunk(geoPackage, FeatureIndexType.GEOPACKAGE, false);
		testIndexChunk(geoPackage, FeatureIndexType.RTREE, true);
	}

	private static void testIndexChunk(GeoPackage geoPackage,
	 ...
	 *             upon error
	 */
	public static void testIndexPagination(GeoPackage geoPackage)
			throws SQLException {
		testIndexPagination(geoPackage, FeatureIndexType.GEOPACKAGE, false);
		testIndexPagination(geoPackage, FeatureIndexType.RTREE, true);
	}

	private static void testIndexPagination(GeoPackage geoPackage,
			FeatureIndexType type, boolean includeEmpty) throws SQLException {
	...
	public static void testLargeIndex(GeoPackage geoPackage, int numFeatures,
		...
		List<FeatureColumn> additionalColumns = GeoPackageTestUtils
				.getFeatureColumns();
		geoPackage.createFeatureTable(FeatureTableMetadata
				.create(geometryColumns, additionalColumns, boundingBox));
	...
	private static void testTimedIndex(GeoPackage geoPackage,
		...
		int geometryFeatureCount = featureDao
				.count(featureDao.getGeometryColumnName() + " IS NOT NULL");
		...
		try {
			...
			GeometryTransform transformToWebMercator = GeometryTransform
					.create(projection, webMercatorProjection);
			GeometryTransform transformToProjection = GeometryTransform
					.create(webMercatorProjection, projection);
